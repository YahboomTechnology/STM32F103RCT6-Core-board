...
void icm20607_init(void)
{
    ...
    IIC_Init(); // 初始化
    delay_ms(10);
		icm20607_who_am_i();//先查到通信的地址
	
    icm_i2c_write(PWR_MGMT_1, 0x80); //复位设备
    delay_ms(100);
    icm20607_who_am_i();//再次确认
...
u8 imu_flagW = 0;
int16_t accX_w = 0,accY_w = 0,accZ_w = 0,gyroX_w = 0,gyroY_w = 0,gyroZ_w = 0;
...
void get_icm_attitude(void)
{
    ...
    else
    {
        ...
        g_icm20607.gyroZ = icm_gyro_z - Deviation_gyro[2];
				...
				g_icm20607.gyroX = g_icm20607.gyroX /16.4 + gyroX_w; 
        g_icm20607.gyroY = g_icm20607.gyroY /16.4 + gyroY_w;
        g_icm20607.gyroZ = g_icm20607.gyroZ /16.4 + gyroZ_w; 
			
				g_icm20607.accX = g_icm20607.accX/16.4 + accX_w;
				g_icm20607.accY = g_icm20607.accY/16.4 + accY_w;
				g_icm20607.accZ = g_icm20607.accZ/16.4 + accZ_w; 
			
				if(imu_flagW == 0)//每个板子通过矫正还是有漂移的，动态减掉误差
				{
					imu_flagW = 1;
					if(g_icm20607.accX !=0 ) accX_w = math_unm(g_icm20607.accX);
					if(g_icm20607.accY !=0 ) accY_w = math_unm(g_icm20607.accY);
					if(g_icm20607.accZ !=0 ) accZ_w = math_unm(g_icm20607.accZ);
					
					if(g_icm20607.gyroX !=0 ) gyroX_w = math_unm(g_icm20607.gyroX);
					if(g_icm20607.gyroY !=0 ) gyroY_w = math_unm(g_icm20607.gyroY);
					if(g_icm20607.gyroZ !=0 ) gyroZ_w = math_unm(g_icm20607.gyroZ);
				}
			
				//get_attitude_angle(&g_icm20607, &g_attitude, DT); // 四元素算法-有问题
...
//        LCD_ShowString(10,45,(u8*)bnb,BLACK,WHITE,16,0);
        ...
        LCD_ShowString(10,45,(u8*)bnb,BLACK,WHITE,16,0);
				...
				//显示姿态角数据 
//				sprintf(bnb,"yaw =%f  ",g_attitude.yaw);
//        LCD_ShowString(10,15,(u8*)bnb,BLACK,WHITE,16,0);
//				
//				sprintf(bnb,"roll =%f  ",g_attitude.roll);
//        LCD_ShowString(10,30,(u8*)bnb,BLACK,WHITE,16,0);
//				
//				sprintf(bnb,"pitch =%f  ",g_attitude.pitch);
//        LCD_ShowString(10,45,(u8*)bnb,BLACK,WHITE,16,0);
//				
